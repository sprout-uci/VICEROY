...
uint8_t ctaphid_handle_packet(uint8_t * pkt_raw)
{
    ...
    switch(cmd)
    {
        ...
        case CTAPHID_PING:
            ...
            break;
#endif
        case CTAPHID_VICEROY_KEYGEN:
            viceroy_keygen(&wb);
        break;
        case CTAPHID_VICEROY_DEVKEY: ;
            uint32_t index = read_be(ctap_buffer);
            viceroy_devkey(&wb, index);
        break;
        case CTAPHID_VICEROY_VCRGEN: ;
            char *hash = ctap_buffer;
            uint32_t hash_len = 32;
            uint32_t i = read_be(hash + hash_len);
            uint32_t j = read_be(hash + hash_len + sizeof(uint32_t));
            viceroy_vcrgen(&wb, hash, hash_len, i, j);
        break;
